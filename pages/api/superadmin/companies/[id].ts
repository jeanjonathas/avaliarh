...
async function exportCompanyData(companyId: string) {
  try {
    ...
    const tests = 
    console.log(`[TEST] Iniciando busca de test (${new Date().toISOString()})`);
    ...
    // Buscar respostas
    const responses = 
    console.log(`[RESPONSE] Iniciando busca de response (${new Date().toISOString()})`);
    ...
    const students = 
    console.log(`[STUDENT] Iniciando busca de student (${new Date().toISOString()})`);
    ...
    console.log(`[TESTINVITATION] Iniciando busca de testInvitation (${new Date().toISOString()})`);
    
    // Forçar desconexão e reconexão para garantir dados frescos
    await reconnectPrisma();
    
    await prisma.testInvitation.findMany({
      where: { companyId },
      include: {
        candidate: true,
        test: true
