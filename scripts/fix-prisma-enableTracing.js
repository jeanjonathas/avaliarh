...
function fixFile(filePath) {
  ...
  // Padrão para encontrar inicializações do PrismaClient sem configuração
  const pattern = /new PrismaClient\(\)/g;
  
  if (pattern.test(content)) {
    console.log(`  Corrigindo: ${filePath}`);
    
    // Substituir por inicialização com configuração enableTracing
